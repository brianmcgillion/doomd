...
#+END_SRC
* Random
# (defun doom/convert-package-pins-to-bump ()
#   "Convert a `package!' call into 'username/repo@commit'.
# For bump commits."
#   (interactive)
#   (save-excursion
#     (beginning-of-line)
#     (let (beg end package pin)
#       (when (re-search-forward "(package! +\\([^ ]+\\)" (line-end-position) t)
#         (goto-char (match-beginning 0))
#         (let* ((end (cdr (bounds-of-thing-at-point 'sexp)))
#                (sexp (sexp-at-point))
#                (recipe (plist-get sexp :recipe)))
#           (setq package (cadr sexp)
#                 pin (plist-get sexp :pin))
#           (goto-char (match-beginning 0))
#           (delete-region (match-beginning 0) end)
#           (insert
#            (format "%s@%s"
#                    (plist-get (doom-plist-merge
#                                (plist-get (cdr recipe) :recipe)
#                                (or (cdr (straight-recipes-retrieve package))
#                                    (plist-get (cdr (assq package
#                                                          doom-packages))
#                                               :recipe)))
#                               :repo)
#                    (substring-no-properties pin 0 7))))))))

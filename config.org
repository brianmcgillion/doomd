...
;; e.g. 1080p
(setq +dashboard-banner-file (concat doom-private-dir "banner/trancendent-gnu.png"))
...
(setopt ispell-dictionary (if IS-MAC "en_GB" "en_US"))
...
;; Pre-warm the citar bibliography cache after idle so that the first
;; call to citar-open-notes doesn't block while parsing the bib file.
;; In daemon mode, defer until after the first client frame connects
;; so the parsing doesn't block emacsclient from attaching.
;; (defun bmg/citar-warm-cache ()
;;   "Pre-warm citar bibliography cache after a short idle delay."
;;   (run-with-idle-timer 2 nil
;;     (lambda ()
;;       (require 'citar)
;;       (citar-cache--get-bibliographies
;;        (citar--bibliography-files) 'global))))

;; (if (daemonp)
;;     (add-hook 'server-after-make-frame-hook
;;               (defun bmg/citar-warm-cache-once ()
;;                 (bmg/citar-warm-cache)
;;                 (remove-hook 'server-after-make-frame-hook
;;                              #'bmg/citar-warm-cache-once)))
;;   (bmg/citar-warm-cache))

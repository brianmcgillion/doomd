...
;; `doom-comint-truncate-buffer-h', governed by `comint-buffer-maximum-size',
;; which it sets to 2048 globally so a chatty process cannot grind the GC.
;; Correct for a long-running shell, wrong for grep
;;
;; Buffer-local, so shells keep Doom's cap.  Deliberately not unlimited: the
;; hook only fires above (* 80 comint-buffer-maximum-size) bytes, so 100000
;; leaves ~8MB of headroom -- far beyond a full-corpus grep (453K) while still
;; bounding a runaway compile.  `grep-mode' derives from `compilation-mode', so
...
Only the timing delays are rebound.  Doom's 0.2 and 0.1
(vertico/config.el:129-132) are tuned for a local `rg'; one rga pass over
these roots measures 1.39s warm, so every pause in typing would kill and
restart a second of work.  min-input stays at Doom's 2, matching
`consult-ripgrep', so two-letter acronyms still search.  `consult--grep'
builds the pipeline synchronously, so a `let' around it reaches these."
  ...
  (let* ((roots (and (consp paths) (stringp (car paths)) paths))
    ...
    (consult--grep prompt #'bmg/consult--rga-make-builder paths initial)))
...
;; Magit's status buffer does not list branches -- `magit-insert-local-branches'
;; belongs to `magit-refs-mode' (the `y' buffer) and is absent from magit's own
;; default `magit-status-sections-hook'.  Add it after Worktrees, since branches
;; and the worktrees that check them out read well together.
;;
(with-eval-after-load 'magit
  (magit-add-section-hook 'magit-status-sections-hook
...
An async consult search: results narrow as you type, text hits preview live
and jump to the exact line.  `C-c C-;' exports to the `grep-mode' buffer
this command used to produce outright, `C-c C-e' to a wgrep-editable one.

consult prefills the minibuffer with `#', so `neural#2024' sends only
`neural' to rga and filters `2024' in Emacs -- free refinement against a
corpus that takes ~1.4s a pass.  But the filter only sees what rga returned,
and rga caps at 5 hits per file, so `neural#transformer' can find nothing in
a paper matching both.  Raise the cap before narrowing when the search must
be exhaustive.

Bare flags after the query go to rga: `neural --glob *.pdf',
`neural --max-count=200', `neural -i'.  A literal `--' does the opposite,
ending the option section and returning the rest to the search string; a
query needing a dashed word escapes it as `\\-'.

With a prefix argument, choose the roots instead.  PDF and EPUB hits open at
the top of the document -- rga's line numbers index extracted text, not a
position Emacs can visit.  PATHS and INITIAL are for programmatic callers."
  (interactive "P")

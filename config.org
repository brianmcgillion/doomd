...
;; Document search, shared by the rga commands in the Org section and by the
;; retrieval in llm-config.el.  Defined here because that file is `load!'-ed
;; from the LLM section, which comes first.
;;
...
`--no-heading --with-filename --line-number' give `grep-mode' the
file:line:text shape it needs for RET to jump to a hit.")
...
(defun bmg/rga--roots (&rest extra)
  "Return shell-quoted search roots: EXTRA plus `bmg/document-search-roots'.
Nil entries in EXTRA are dropped, so an unset bibliography is harmless."
  (mapconcat (lambda (d) (shell-quote-argument (expand-file-name d)))
             (delq nil (append extra bmg/document-search-roots))
             " "))
...
well as text, and the old version produced output nothing could navigate."
  ...
  (compilation-start
   (format "rga %s -i %s %s"
           bmg/rga-common-args
           (shell-quote-argument query)
           ;; Notes and the bibliography, then the document roots.
           ;; citar-bibliography can legitimately be empty.
           (bmg/rga--roots org-roam-directory (car citar-bibliography)))
   'grep-mode
